...
app.use('/dist', express.static(frontEnd))
app.use('/vendor', express.static(frontEnd1))
...
app.post('/node/dynamicForms/getJsonFormResponse', async (req, res) => {
  ...
  // console.log('132-ram', JSON.stringify(req.query))

  let formData = JSON.parse(req.body.data);
  // console.log(JSON.parse(summary),'268');
  ...
  let isLatLng = _.findWhere(formSchema.Item.fields, { "field_type": "location" });
  let mergeLocation = {};
  if (isLatLng !== undefined) {
  ...
  if (isSignIndex !== undefined) {
    if (isSignIndex.required) {
      if (req.body.sign) {
        // var file = new File([myBlob], "name");
        console.log(req.body.sign, 'test')
        sign[`${isSignIndex.label}`] = req.body.sign;

      } else {
        return res.status(200).send({
          success: false,
          data: isSignIndex.label + ' is Mandatory for this form.'
        })
      }
    } else {
      if (req.body.sign) {
  ...
  Request.post({
    "headers": header,
    "url": apiDomain + "api/eForms/UpdateFormResponse?formResponseId=" + formResponseId + "&securityCode=" + code,
